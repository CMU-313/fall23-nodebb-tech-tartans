...
describe('User', () => {
    ...
    describe('API Endpoint: POST /users/:userId/addfriends', () => {
        // Positive Test Case: Adding Friends Successfully
        it('should add friends successfully adding multiple friends', (done) => {
            const data = {
                friends_data: [[1, 'admin'], ['2, yuvanshu']],
            };
    
            api.post(`/users/${app.user.uid}/addfriends`, data)
                .then((response) => {
                    assert.strictEqual(response.status, 200); // Check for a successful HTTP status code
                    assert.strictEqual(response.body, 'Friends added successfully'); // Check the response message
                    done();
                })
                .catch(done);
        });
        ...
        // Positive Test Case: Adding Friends Successfully
        it('should add friends successfully adding no friends', (done) => {
            const data = {
                friends_data: [],
            };
        ...
        // Negative Test Case: Friends Not Added Successfully
        it('should handle inaccurate data types', (done) => {
            const data = {
                friends_data: [['admin']],
            };
            ...
            api.post(`/users/${app.user.uid}/addfriends`, data)
                .then((response) => {
                    assert.strictEqual(response.status, 500); // Check for an internal server error status code
                    assert.strictEqual(response.body, 'Internal Server Error'); // Check the response message
                    done();
                })
                .catch(done);
        });
        ...
        // Negative Test Case: Friends Not Added Successfully
        it('should handle inaccurate length of friends_data pair', (done) => {
            const data = {
                friends_data: [[1]],
            };
